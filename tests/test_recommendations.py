...
class TestInvisibleDoctor:
    ...
    def test_invisibility_warning(self):
        ...
        score = ScoreBreakdown(
            visibility=0.0, dominance=100.0, indirect_presence=0.0, overall=10.0,
        )
        recs = generate_recommendations(verdicts, score, "Dr. Teste", "Dermatologia")

        assert any("invisível" in r.lower() for r in recs)
        assert any("construir" in r.lower() or "presença" in r.lower() for r in recs)
...
class TestCompetitorDominance:
    ...
    def test_names_top_competitor(self):
        ...
        score = ScoreBreakdown(
            visibility=0.0, dominance=20.0, indirect_presence=0.0, overall=5.2,
        )
        recs = generate_recommendations(verdicts, score, "Dr. Teste", "Dermatologia")
...
class TestAboveBenchmark:
    ...
    def test_positive_message(self):
        ...
        score = ScoreBreakdown(
            visibility=100.0, dominance=100.0, indirect_presence=0.0, overall=100.0,
        )
        recs = generate_recommendations(verdicts, score, "Dr. Teste", "Dermatologia")
...
class TestBelowBenchmark:
    ...
    def test_benchmark_comparison(self):
        ...
        score = ScoreBreakdown(
            visibility=10.0, dominance=100.0, indirect_presence=0.0, overall=24.0,
        )
        recs = generate_recommendations(verdicts, score, "Dr. Teste", "Dermatologia")
