...
Formula from SPEC §7.4:
- presence: % of prompts where doctor appeared (any form)
- quality:  weighted average by citation type × confidence
- position: higher rank = higher score (when mentioned by name)
- competitive: inverse of competitor displacement rate
...
# Specialty benchmarks (from PRD — typical AI visibility scores)
...
def score(verdicts: list[Verdict]) -> ScoreBreakdown:
    ...
    # Presence: % of prompts where doctor appeared (mentioned_by_name or mentioned_as_specialty)
    mentioned = [
        v
        for v in verdicts
    ...
    # Quality: weighted average of citation quality × confidence
    quality = (
        sum(QUALITY_VALUE[v.citation_type] * v.confidence for v in verdicts) / n
    )

    # Position: when mentioned by name, earlier position = higher score
    by_name = [
        v
        for v in verdicts
    ...
    if by_name:
        # Position 1 → 100pts, position 2 → 90pts, ... position 10 → 10pts, 11+ → 0
        position = (
...
def get_benchmark(specialty: str) -> float:
    """Return the specialty benchmark score, or the default."""
